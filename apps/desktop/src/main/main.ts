...
// Push orchestrator status to renderer
onStatus((status) => {
  BrowserWindow.getAllWindows().forEach(win => {
    win.webContents.send('orchestrator:status', status)
  })
})

ipcMain.handle('orchestrator:start', async (_event, config: OrchestratorConfig) => {
  if (isRunning()) return { success: false, error: 'Already running' }
  // Don't await — runs in background
  startOrchestrator(config).catch(console.error)
  return { success: true }
})
...
ipcMain.handle('orchestrator:getState', () => {
  return { state: getOrchestratorState(), running: isRunning() }
})


ipcMain.handle('orchestrator:preflight', async () => {
  return runPreflight()
})
